...
Above, we see from "summary(lambdaCalc)" that lambda values (mean, 1st Q. etc.) are almost identical for each case.
...
Above, we can see the Mape results for each hour. First of all, when we compare these results with Part D, we can say that the Mape values are slightly bad for every hour. This may mean that penalized regression model is better than the fused penalty model. However, lambda values create an important difference in fused penalty approach. Therefore, selected lambda values for Part E might be wrong. For some lambda values, fused penalty approach might be slightly better then penalized regression model used in Part D.  We observe a similar difference between working hours and night hours like in Part C and Part D. It is possible to say that this regression model is almost excellent because the Mape values are very low.
...
If we really want to know whether our predictions are good or not, one way to do this is to compare them with the naive approaches. Those predictions with the naive approach can be seen as a baseline. Therefore, we see that the regression model we used in Part D (lasso regression) is the best and is very effective. The main reason for lasso regression having the best performance is because it uses penalized approach. fused approach has also performed well. Probably due to the chosen lambda value, it performed slightly bad compared to Part D. If the lambda value was chosen differently, we might observe an improvement and maybe with this way its performance would be better than Part D.

All in all, it is obvious that there are other regression models or approaches which are better than the naive approach. Thus, for predictions, regression algorithms, especially with penalties, must be used to make really good predictions. Also, parameters such as lambda must be chosen accordingly to get the highest performance.
